...
export function registerDosScenario({ createChannel, updateDiagnosticsDashboard, diagSummary, diagTestActive }) {
  console.log('[dos.js] registerDosScenario called');
  window.runDosTest = async (btn) => {
    console.log('[dos.js] runDosTest triggered');
    btn.disabled = true;
    const N = 50000, slots = 16, size = 80;
    let processed = 0, sent = 0, timeouts = 0;
    const { channel } = createChannel({ slots, slotSize: size, sweepTimeoutMs: 40 });
    const start = Date.now();
    let producerDone = false;
    // Producer: sendAsync in a loop, but yield and update UI every 1000
    const producer = (async () => {
      for (let i = 0; i < N; i++) {
        const ok = await channel.sendAsync(encode({ i }), undefined, { timeoutMs: 10000 });
        if (!ok) timeouts++;
        sent++;
        if (i % 1000 === 0) {
          document.getElementById("dos_output").textContent = `Sent: ${sent}  Processed: ${processed}  Timeouts: ${timeouts}`;
    ...
    document.getElementById("dos_result").innerHTML =
      ...
      `Processed ${processed}/${N} (${((processed / N) * 100).toFixed(1)}%)</span>`;
